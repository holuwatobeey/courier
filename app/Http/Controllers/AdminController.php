...
class AdminController extends Controller
{
    ...
    {
        $users = DB::table('users')->orderBy('id', 'desc')->where('role', 0)->paginate(10);
        $dms = DB::table('users')->orderBy('id', 'desc')->where('role', 2)->paginate(10);
        return view('see-users')->with(array('users'=>$users, 'dms'=>$dms));
    }
    ...
    public function add_user(Request $request){
        ...
        if(Input::has('delete')){
            ...
            $user->delete();
            return redirect('/see-users')->with('status', 'Delivery Master Deleted Successfully!');
        }
